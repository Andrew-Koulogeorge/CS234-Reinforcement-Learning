...
class PolicyGradient(object):
    ...
    def init_policy(self): # ??? correct ??? 
        ...
        input_size, output_size, n_layers, size = self.observation_dim, self.action_dim, self.config.n_layers, self.config.layer_size

        network = build_mlp(input_size, output_size, n_layers, size)

        if self.discrete: self.policy = CategoricalPolicy(network)
        else: self.policy = GaussianPolicy(network, action_dim=self.action_dim)

        self.optimizer = torch.optim.Adam(params=self.policy.parameters(), lr=self.config.learning_rate)
    ...
    def sample_path(self, env, num_episodes=None):
        ...
        while num_episodes or t < self.config.batch_size: # looping over total number of trajectories
            state = env.reset()
            states, actions, rewards = [], [], [] # hold states, actions and rewards from this trajectory
            episode_reward = 0 # init value of total episode reward 0

            for step in range(self.config.max_ep_len): # looping over max length of a given trajectory
                states.append(state)
                action = self.policy.act(states[-1][None])[0] # sample from the distribution over actions given by policy
                state, reward, done, info = env.step(action) # apply this action in env and observe next state and reward
                actions.append(action) # append actions
                rewards.append(reward) # append reward
                episode_reward += reward # add reward gained to total count
                t += 1
                if done or step == self.config.max_ep_len - 1: # if terminal state reached or on final time step
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            returns = [0] * len(rewards) # array to hold returns from current path
            returns[-1] = rewards[-1] # base case 

            for i in range(len(rewards)-2, -1, -1): # looping over rewards from end of trajectory to start
    
                returns[i] = rewards[i] + self.config.gamma * returns[i+1] # current return = current_reward + gamma * reward_next_time_step
    ...
    def update_policy(self, observations, actions, advantages):
        ...
        # for each observation, what is the distribution over actions we could take
        action_distribution = self.policy.action_distribution(observations)

        # for each action we actually look, get the probability density for that action
        log_pi_a_given_s = action_distribution.log_prob(actions)

        # since we want to maximize this probability, make loss negative. 
        # LOSS FUNCTION HAS TO BE A SCALAR FOR US TO COMPUTE GRADIENT --> TAKE MEAN ACROSS BATCH EXAMPLES!
        loss =  -1 * torch.mean(advantages * log_pi_a_given_s) 

        self.optimizer.zero_grad() # ensure gradients are not accumulating across batch examples
        loss.backward() # compute the gradient of the loss by using backward()
        self.optimizer.step() # step with the adam optimizer to change the weights of the policy
    ...
    def train(self):
        ...
        # self.logger.info("- Exporting plot")
        export_plot(
