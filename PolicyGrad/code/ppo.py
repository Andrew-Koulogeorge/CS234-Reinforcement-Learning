...
class PPO(PolicyGradient):
    ...
    def update_policy(self, observations, actions, advantages, old_logprobs):
        ...
        # use observations to gather current distributions over actions
        distributions = self.policy.action_distribution(observations)

        # get new_logprobs based on distributions and actions taken
        new_logprobs = distributions.log_prob(actions)

        r_t = torch.exp(new_logprobs - old_logprobs) # exp of new and old log probs to get ratio 
        clipped_r_t = torch.clamp(r_t, 1 - self.eps_clip, 1 + self.eps_clip)

        obj = r_t * advantages
        obj_clipped = clipped_r_t * advantages

        loss_clip = -torch.mean(torch.min(obj, obj_clipped)) # ppo clip loss

        self.optimizer.zero_grad() # ensure gradients are not accumulating
        loss_clip.backward() # compute gradient wrt clip loss
