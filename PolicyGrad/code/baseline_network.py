...
class BaselineNetwork(nn.Module):
    ...
    def __init__(self, env, config):
        ...
        """
        super().__init__()
        self.config = config # stores hyper parameters
        self.env = env # from gym to act in the world, get next states and reward
        self.baseline = None 
        self.lr = self.config.learning_rate
        observation_dim = self.env.observation_space.shape[0] # vector size of state

        #######################################################
        #########   YOUR CODE HERE - 2-8 lines.   #############

        SCALAR_OUTPUT = 1 # baseline network is estimating the expected return from this state
        self.network = build_mlp(input_size=observation_dim, output_size=SCALAR_OUTPUT, n_layers=self.config.n_layers, size=self.config.layer_size)
    ...
    def calculate_advantage(self, returns, observations):
        ...
        # returns are ALL of the returns at each time step. Output of network is just a scalar for each observation

        advantages = (returns - self.network(observations).detach().numpy())
    ...
    def update_baseline(self, returns, observations):
        ...
        # run forward pass for the baseline model to compute approximation for value of (state,action)
        baseline_estimate = self.network(observations)

        # compute MSE between baseline estimate and real returns computed at each time step
        loss = torch.mean((returns - baseline_estimate) ** 2)
