...
class LinearSchedule(object):
    def __init__(self, eps_begin, eps_end, nsteps):
        """
        Args:
            eps_begin: initial exploration --> How much are we going to be exploring the action space when we start training?
            eps_end: end exploration --> How much are we going to be exploring once we have already learned a little bit about the space?
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action: int) -> int:
        ...
        # check to see if epsilon <= r
        # if so, randomly select an action 
